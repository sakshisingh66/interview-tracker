...
exports.createQuestion = aEH(async (req, res, next) => {
    const {title,topic,link} = req.body;
    const description = req.body.description || '';
    if (!title || !topic  || !link ) return next(new Err('Enter all fields', 400));
    
    // const index = 1;
    const question = await Question.create({title,topic,link,description});
    question.save();
    res.status(200).json({
        status: 'success',
        data: { question },
